...
| H2 | sub-agent-container | `spec.py:60-62` | **`container_expert` allowlist includes `bash_exec` + `ls` (raw host shell).** — ✅ **ACCEPTED RISK** (owner decision, 2026-07-06). The child-guard (`_mitigation/07`) already blocks the catastrophic commands (rm/dd/docker/…); the residual is host-file reads + output egress to the provider — acceptable for a single trusted operator, and `bash_exec`'s open-ended flexibility is needed because dispatches are stateless (no cross-dispatch memory). **Revisit only if this sub-agent is ever pointed at untrusted/adversarial input.** Scoped-tool replacement (`write_build_file`/`http_check`) is the fix if that changes. |
