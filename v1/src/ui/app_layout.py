...
def build_app(
    ...
    app = Application(
        ...
        full_screen=True,
        # mouse_support=False so the terminal handles mouse events natively:
        # - Click-drag selects text (so users can copy session IDs, code, etc.)
        # - Triple-click selects a line
        # - Cmd/Ctrl+click works as usual
        # - Mouse scroll-wheel uses the terminal's scrollback buffer
        # Trade-off: scroll wheel can't drive the conversation model's scroll
        # state (would require capturing mouse events). Conversation scroll is
        # handled by keyboard instead — PageUp/PageDown and ↑/↓ (with empty
        # input) are bound in app_keybindings.py.
        mouse_support=False,
